...
(function(){
	...
	extend(Chart.Type.prototype,{
		...
		resize : function(callback){
			...
			//alert(maxWidth+" "+newWidth+" h:"+newHeight+" "+maxHeight);
			canvas.width = this.chart.width = newWidth;
			canvas.height = this.chart.height = newHeight;
			//alert(canvas.width+" h:"+canvas.height);
	...
	Chart.Point = Chart.Element.extend({
		...
		inRange: function(chartX,chartY){
			...
			//alert("detr: " +  this.hitDetectionRadius + " r: " + this.radius);
	...
	Chart.Scale = Chart.Element.extend({
		...
		calculateYREV : function(value){
			var scalingFactor = this.drawingArea() / (this.min - this.max);
			return ((this.endPoint - value)/scalingFactor) +  this.min;
		},
		...
		calculateXREVS : function(valueOffset){
			var innerWidth = this.width - (this.xScalePaddingLeft + this.xScalePaddingRight),
			valueWidth = innerWidth/Math.max((this.valuesCount - ((this.offsetGridLines) ? 0 : 1)), 1),
			index = ((valueOffset -  this.xScalePaddingLeft) / valueWidth);
	...
	Chart.Type.extend({
		...
		initialize:  function(data){
			...
			helpers.each(data.datasets,function(dataset){
				...
				helpers.each(dataset.data,function(dPoint,index){
					var dataPoint = dPoint[0];
					//Add a new point for each piece of data, passing any required data to draw.
					datasetObject.points.push(new this.PointClass({
						value : dataPoint / 255 * 100,
		...
		draw : function(ease){
			...
			helpers.each(this.datasets,function(dataset){
				...
				helpers.each(dataset.points, function(point, index){
					//alert("P: " + point + " I: " + index);
					if (point.hasValue()){
						point.transition({
							y : this.scale.calculateY(point.value),
							x : this.scale.calculateX(point.value2/point.step)
						}, easingDecimal);
					}
				},this);
