...
private:
  ...
  byte currentLightMode =
      0; // 0=high sun, 1=mid sun, 2=low sun, 3=moon, 4=transition, 5=unknown
  ...
  LightColor tempColor = {
      0, 0, 0,
      0, 0, 0}; // The temporary color of the light (used for updating sat+)
  ...
  LightColor lightMoon = {0, 0, 0, 0,
                          0, 0}; // store the RGBW values for the CS+ M4 button

  // red, green, blue, white, yellow, violet, aqua
  const unsigned int VGAColor[7] = {0xF800, 0x07E0, 0x001F, 0xFFFF,
                                    0xFFE0, 0xF81F, 0x07FF};

  // store colours for bargraphs
  byte barColors[6] = {0, 1, 2, 3, 4, 5};
  byte selectedChan =
      0; // used for tracking which channel is selected when changing colors
  ...
  struct PWRSCHED {
    ...
    byte offHour;
    byte offMinute;
  } schedLights1, schedLights1s2, schedLights2, schedLights2s2, schedCirc,
      schedCo2, schedAux1, schedAux2, ramp1, ramp2, ramp3, ramp4, ramp5, ramp6;
  ...
  // heater variables
  boolean heaterWarning =
      false; // keeps track if there is an active overheating issue
  boolean heaterWarningCleared =
      true; // keeps track if we clear the warning, impacts home screen icon
  byte heatOffTemp = 30;
  byte heatOnTemp = 25;
  ...
  unsigned long ATORunTime =
      120; // seconds to run ATO pump for before flagging an alarm
...
#define SHORT_WAIT 25
  ...
  // time variables to track clock updating, dimming timing and time to return
  // home from a menu
  ...
  boolean backlightTouch =
      true; // initial setting of true to start the screen bright after boot
  ...
  // If PCA9865 is installed this is true, otherwise it is false to use Arduino
  // PWM pins
  ...
  char *lightModeS[4] = {"5hsunS.raw", "5msunS.raw", "5lsunS.raw",
                         "5moonS.raw"};
  ...
  // large power buttons for the power screen and the feeding configuration
  // screen, off and on
  ...
  // on off power dot under each power button on the power screen and feeding
  // config screen
  ...
  char *moonImages[8] = {"21new.raw",   "21ncres.raw", "21firstq.raw",
                         "21waxg.raw",  "21full.raw",  "21wang.raw",
...
public:
  ...
  void graphChannel(int lowSunVal, int midSunVal, int highSunVal, int moonVal,
                    byte channel, int x, int y, int dx, int dy);
