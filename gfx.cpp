...
LCD_HAL_Interface::LCD_HAL_Interface(Adafruit_ILI9341 &t) : tft(t) {}

void LCD_HAL_Interface::fillScreen(int a) const { tft.fillScreen(a); }

void LCD_HAL_Interface::drawText(int x, int y, const char *c, int d, int e,
                                 int f) const {
  tft.setCursor(x, y);
  tft.setTextColor(d);
  tft.setTextSize(f);
...
int LCD_HAL_Interface::getWidth() const { tft.getWidth(); }

int LCD_HAL_Interface::getHeight() const { tft.getHeight(); }

void LCD_HAL_Interface::drawCircle(int x, int y, int s, int color) const {
  tft.fillRect(x, y, s, s, color);
}

void LCD_HAL_Interface::fillCircle(int x, int y, int s, int color) const {
  tft.fillRect(x, y, s, s, color);
}
